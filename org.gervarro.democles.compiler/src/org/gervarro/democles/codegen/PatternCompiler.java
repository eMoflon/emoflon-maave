...
public class PatternCompiler<T extends TemplateEngine> {
	...
	public PatternCompiler(final T engine, final OperationSequenceCompiler operationCompiler) {
		this.engine = engine;
	...
	protected final MainTemplateController getPatternDataFrameGenerator(CompilerPattern pattern) throws Exception {
		...
		final DefaultImportManager importManager = new DefaultImportManager(packageName, className);
	...
	protected final MainTemplateController getSimplePatternGenerator(CompilerPattern pattern, Adornment... adornments)
			throws Exception {
		final String packageName = pattern.getBuilder().getPackageName();
		...
		Chain<TemplateController> matchSingleSuccess = new Chain<TemplateController>(
				new TemplateController("CompiledPattern/SuccessfulSingleMatching"));

		Chain<TemplateController> matchAllSuccess = new Chain<TemplateController>(
				new TemplateController("CompiledPattern/SuccessfulAllMatching"));

		List<TemplateInvocation> methods = new LinkedList<TemplateInvocation>();
		for (Adornment adornment : adornments) {
			final Chain<GeneratorOperation> reverseSearchPlan = module.generateReverseSearchPlan(body.getOperations(),
					body.calculateAdornment(adornment));
			methods.add(preparePatternMatchingMethodGenerator(importManager, pattern, adornment, body,
					"CompiledPattern/MatchSingleMethod", reverseSearchPlan, matchSingleSuccess));
			methods.add(preparePatternMatchingMethodGenerator(importManager, pattern, adornment, body,
					"CompiledPattern/MatchAllMethod", reverseSearchPlan, matchAllSuccess));
		}
	...
	protected final MainTemplateController getRegularPatternGenerator(CompilerPattern pattern, Adornment... adornments)
			throws Exception {
		final String packageName = pattern.getBuilder().getPackageName();
		final String className = pattern.getClassName();
		final DefaultImportManager importManager = new DefaultImportManager(packageName, className);
	...
	protected final MainTemplateController getAbstractPatternBodyGenerator(CompilerPattern pattern,
			Adornment... adornments) throws Exception {
		final String packageName = pattern.getBuilder().getPackageName();
	...
	protected final MainTemplateController getPatternBodyGenerator(CompilerPatternBody body, Adornment... adornments)
			throws Exception {
		final CompilerPatternBuilder module = body.getHeader().getBuilder();
		...
		List<TemplateInvocation> methods = new LinkedList<TemplateInvocation>();
		for (Adornment adornment : adornments) {
			Chain<GeneratorOperation> reverseSearchPlan = module.generateReverseSearchPlan(body.getOperations(),
					body.calculateAdornment(adornment));
			methods.add(preparePatternMatchingMethodGenerator(importManager, body.getHeader(), adornment, body,
					"CompiledPattern/MatchSingleMethod", reverseSearchPlan, matchSingleSuccess));
			methods.add(preparePatternMatchingMethodGenerator(importManager, body.getHeader(), adornment, body,
					"CompiledPattern/MatchAllMethod", reverseSearchPlan, matchAllSuccess));
		}
	...
	public final TemplateInvocation preparePatternMatchingMethodGenerator(ImportManager importManager,
			CompilerPattern pattern, Adornment adornment, CompilerPatternBody body, String methodTemplate,
			Chain<GeneratorOperation> operationChain, Chain<TemplateController> tail) throws Exception {
