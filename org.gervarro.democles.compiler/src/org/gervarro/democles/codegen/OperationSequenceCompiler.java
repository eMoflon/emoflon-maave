...
public final class OperationSequenceCompiler {
	...
	public final Chain<TemplateController> buildOperationChain(Chain<GeneratorOperation> operationChain, Chain<TemplateController> tail) {
		...
		GeneratorOperation operation = operationChain.getValue();
		for (CodeGeneratorProvider<Chain<TemplateController>> constraintTypeSpecific : constraintTypeSpecificProviders) {
			if (constraintTypeSpecific.isResponsibleFor(operation)) {
				tail = constraintTypeSpecific.getTemplateController(operation, tail);
				break;
			}
		}
		return buildOperationChain(operationChain.getNext(), tail);
	}
}
