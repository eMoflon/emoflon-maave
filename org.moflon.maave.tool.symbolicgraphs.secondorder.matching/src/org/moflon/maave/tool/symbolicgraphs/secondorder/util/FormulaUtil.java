...
public class FormulaUtil {
					...
					for (Parameter param_from : pred_from.getParameters()) {
						if (param_from instanceof LabelNode) {
							...
							if (ln_from.eContainer() == quantifier_from) {
								targetLn = quantifier_from_new.getLabelNodes()
										.get(quantifier_from.getLabelNodes().indexOf(param_from));
							} else {
					...
					for (Parameter param_to : pred_to.getParameters()) {
						if (param_to instanceof LabelNode) {
							...
							if (ln_to.eContainer() == quantifier_to) {
								targetLn = quantifier_to_new.getLabelNodes()
										.get(quantifier_to.getLabelNodes().indexOf(param_to));
							} else {
								targetLn = ln_to;
							}
							pred_new.getParameters().add(targetLn);
	...
	public static void disjunctDomFormulawithCodomFormula(SymbolicGraphMorphism mor) {
		...
		List<LabelNode> removedLnList_codom = mor.getCodom().getLabelNodes().stream()
				.filter(ln -> (mor.isInImage(ln) == false)).collect(Collectors.toList());
			...
			for (Predicate pred_codom : conj_codom.getOf()) {
				...
				for (Parameter param_codom : pred_codom.getParameters()) {
					if (param_codom instanceof LabelNode) {
						...
						LabelNode ln_dom = dom.getLabelNodes().stream().filter(ln -> mor.imageOf(ln) == ln_codom)
								.findAny().orElse(null);
						if (ln_dom == null) {
							ln_dom = quantifier_dom.getLabelNodes().stream()
									.filter(ln -> quantLnMapDom_Codom.get(ln) == param_codom).findAny().orElse(null);
						}
	...
	private static void syntacticallySimplifyFormula(Disjunction phi) {
		...
		for (Conjunction conj : conjunctions) {
			if (conj.getOf().stream().anyMatch(pred -> pred.getSymbol().equals("#F"))) {
				...
				List<Predicate> predList = conj.getOf().stream().filter(pred -> pred.getSymbol().equals("#T"))
