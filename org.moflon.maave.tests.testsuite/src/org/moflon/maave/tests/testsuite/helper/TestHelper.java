...
public class TestHelper {
	...
	public static boolean checkMorphismsInRule(SymbGTRule rule) {
		...
		for (GraphNode gn : K.getGraphNodes()) {
			result = result && (left.imageOf(gn) != null) && (left.imageOf(gn).getDebugId().equals(gn.getDebugId()))
					&& (right.imageOf(gn) != null) && (right.imageOf(gn).getDebugId().equals(gn.getDebugId()));
		}
		for (GraphEdge ge : K.getGraphEdges()) {
			result = result && (left.imageOf(ge) != null) && (left.imageOf(ge).getDebugId().equals(ge.getDebugId()))
					&& (right.imageOf(ge) != null) && (right.imageOf(ge).getDebugId().equals(ge.getDebugId()));
		}
		for (LabelNode ln : K.getLabelNodes()) {
			result = result && (left.imageOf(ln) != null) && (left.imageOf(ln).getLabel().equals(ln.getLabel()))
					&& (right.imageOf(ln) != null) && (right.imageOf(ln).getLabel().equals(ln.getLabel()));
		}
		for (LabelEdge le : K.getLabelEdges()) {
			result = result && (left.imageOf(le) != null) && (left.imageOf(le).getDebugId().equals(le.getDebugId()))
					&& (right.imageOf(le) != null) && (right.imageOf(le).getDebugId().equals(le.getDebugId()));
		}
		return result;
	}
	...
	public static boolean checkMorphism(SymbolicGraphMorphism mor) {
		...
		for (GraphNode gn : mor.getDom().getGraphNodes()) {
			result = result && (mor.imageOf(gn) != null) && (mor.imageOf(gn).getDebugId().equals(gn.getDebugId()));

		}
		for (GraphEdge ge : mor.getDom().getGraphEdges()) {
			result = result && (mor.imageOf(ge) != null) && (mor.imageOf(ge).getDebugId().equals(ge.getDebugId()));

		}
		for (LabelNode ln : mor.getDom().getLabelNodes()) {
			result = result && (mor.imageOf(ln) != null) && (mor.imageOf(ln).getLabel().equals(ln.getLabel()));

		}
		for (LabelEdge le : mor.getDom().getLabelEdges()) {
			result = result && (mor.imageOf(le) != null) && (mor.imageOf(le).getDebugId().equals(le.getDebugId()));
		...
		for (GraphNode gn : graphK.getGraphNodes()) {
			for (EAttribute attrib : gn.getType().getEAllAttributes()) {
				if (!((gn.getLabelEdge().stream().filter(le -> le.getType() == attrib).count() <= 1)
						&& gn.getLabelEdge().stream()
								.filter(le -> le.getTarget().getType() == attrib.getEAttributeType()).count() <= 1)) {
					return false;
				}
			}
		}
