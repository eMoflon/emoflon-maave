...
public class EvalHelper {
	public static ACEnforcmentReport mergeReports(List<ACEnforcmentReport> reports) {
		...
		ACEnforcmentReportImpl newReport = (ACEnforcmentReportImpl) AcenforcmentFactory.eINSTANCE
				.createACEnforcmentReport();
		...
		for (SymbGTRule rule : rules) {
			...
			for (ACEnforcmentReport acEnforcmentReport : reports) {
				...
				SymbGTRule currentRule = report.ruleToResultMap.keySet().stream()
						.filter(x -> x.getName().equals(rule.getName())).findAny().get();
				newResult.setNrOfPreAC(
						acEnforcmentReport.lookupResult(currentRule).getNrOfPreAC() + newResult.getNrOfPreAC());
				newResult.setNrOfPreACRaw(
						acEnforcmentReport.lookupResult(currentRule).getNrOfPreACRaw() + newResult.getNrOfPreACRaw());
				newResult.setNrOfPreAcRemGuaranteeingAC(
						acEnforcmentReport.lookupResult(currentRule).getNrOfPreAcRemGuaranteeingAC()
								+ newResult.getNrOfPreAcRemGuaranteeingAC());
				newResult.setTimeConstToPrecond(acEnforcmentReport.lookupResult(currentRule).getTimeConstToPrecond()
						+ newResult.getTimeConstToPrecond());
				newResult.setTimeMinimizeRedAC(acEnforcmentReport.lookupResult(currentRule).getTimeMinimizeRedAC()
						+ newResult.getTimeMinimizeRedAC());
				newResult.setTimeRemoveGuaranteeingAC(
						acEnforcmentReport.lookupResult(currentRule).getTimeRemoveGuaranteeingAC()
								+ newResult.getTimeRemoveGuaranteeingAC());
				newResult.setNrOfPostNACs(
						acEnforcmentReport.lookupResult(currentRule).getNrOfPostNACs() + newResult.getNrOfPostNACs());
				newResult.setTimeForPostNacs(acEnforcmentReport.lookupResult(currentRule).getTimeForPostNacs()
						+ newResult.getTimeForPostNacs());
				newResult.setTimePostToPre(
						acEnforcmentReport.lookupResult(currentRule).getTimePostToPre() + newResult.getTimePostToPre());
			}
	...
	public static void reportToFile(ConfluenceAnalysisReport report) {
		...
		Function<ConfluenceAnalysisResult, String> fTimesL = x -> {
			return "\\makecell{" + x.getCpaResult().getTimeCalcMinContexts() + " | "
					+ x.getCpaResult().getTimeFilteringConsistent() + " | " + x.getCpaResult().getTimeForCalcCritPairs()
		...
		Function<ConfluenceAnalysisResult, String> fNumberVSTimesL = x -> {
			return "\\makecell{" + x.getCpaResult().getNrOfMinContexts() + " | "
					+ x.getCpaResult().getNrOfConsistentMinContexts() + " | " + x.getCpaResult().getNrOfCritPairs()
					+ " | " + x.getNrOfNonConfluentCriticalPairs() + "\\\\" + x.getCpaResult().getTimeCalcMinContexts()
					+ " | " + x.getCpaResult().getTimeFilteringConsistent() + " | "
					+ x.getCpaResult().getTimeForCalcCritPairs() + " | " + x.getTimeForConfluence() + "}";
		...
		try {
			...
			FileUtils.writeStringToFile(new File("instances/TimeOverall.csv"),
					ConfluenceAnalysisResultPrinter.confluenceReportToCSVTable(report, fTimeOverall));

			FileUtils.writeStringToFile(new File("instances/TimeConfluence.csv"),
					ConfluenceAnalysisResultPrinter.confluenceReportToCSVTable(report, fTimeConfluence));

			FileUtils.writeStringToFile(new File("instances/TimeCalcMinContexts.csv"),
					ConfluenceAnalysisResultPrinter.confluenceReportToCSVTable(report, fTimeCalcMinContexts));

			FileUtils.writeStringToFile(new File("instances/TimeFilteringConsistent.csv"),
					ConfluenceAnalysisResultPrinter.confluenceReportToCSVTable(report, fTimeFilteringConsistent));

			FileUtils.writeStringToFile(new File("instances/TimeCalcCritpairs.csv"),
					ConfluenceAnalysisResultPrinter.confluenceReportToCSVTable(report, fTimeCalcCritpairs));

			FileUtils.writeStringToFile(new File("instances/NrMincontexts.csv"),
					ConfluenceAnalysisResultPrinter.confluenceReportToCSVTable(report, fNrMincontexts));

			FileUtils.writeStringToFile(new File("instances/NrOfConsistentMinContexts.csv"),
					ConfluenceAnalysisResultPrinter.confluenceReportToCSVTable(report, fNrOfConsistentMinContexts));

			FileUtils.writeStringToFile(new File("instances/NrOfCritPairs.csv"),
					ConfluenceAnalysisResultPrinter.confluenceReportToCSVTable(report, fNrOfCritPairs));

			FileUtils.writeStringToFile(new File("instances/NrNonConfluentPairs.csv"),
					ConfluenceAnalysisResultPrinter.confluenceReportToCSVTable(report, fNrNonConfluentPairs));

			FileUtils.writeStringToFile(new File("instances/TimeOverall.tex"),
					ConfluenceAnalysisResultPrinter.confluenceReportToLatexTable(report, fTimeOverall));

			FileUtils.writeStringToFile(new File("instances/TimeCPAvsConfluence.tex"),
					ConfluenceAnalysisResultPrinter.confluenceReportToLatexTable(report, fTimeCPAvsConfluenceL));

			FileUtils.writeStringToFile(new File("instances/Numbers.tex"),
					ConfluenceAnalysisResultPrinter.confluenceReportToLatexTable(report, fNumbersL));

			FileUtils.writeStringToFile(new File("instances/Times.tex"),
					ConfluenceAnalysisResultPrinter.confluenceReportToLatexTable(report, fTimesL));

			FileUtils.writeStringToFile(new File("instances/NumberVSTimes.tex"),
					ConfluenceAnalysisResultPrinter.confluenceReportToLatexTable(report, fNumberVSTimesL));

			System.out.println(ConfluenceAnalysisResultPrinter.confluenceReportToLatexTable(report, fconfluentL));

			StringBuilder sb = new StringBuilder();
			sb.append(
					"OverallNrOfMinCtx;OverallNrOfConsistMinCtx;OverallNrOfCritPairs;OverallNrOfNonConfluentCritPairs\n");
			sb.append(report.getOverallNrOfMinCtx() + ";" + report.getOverallNrOfConsistMinCtx() + ";"
					+ report.getOverallNrOfCritPairs() + ";" + report.getOverallNrOfNonConfluentCritPairs() + "\n");
