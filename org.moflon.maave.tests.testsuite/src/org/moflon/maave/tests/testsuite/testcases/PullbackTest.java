...
public class PullbackTest {
	...
	public void test() {
		...
		MoflonEOperation opB = (MoflonEOperation) cls.getEOperations().stream()
				.filter(x -> x.getName().equals("testPatternB")).findFirst().get();
		MoflonEOperation opC = (MoflonEOperation) cls.getEOperations().stream()
				.filter(x -> x.getName().equals("testPatternC")).findFirst().get();
		MoflonEOperation opD = (MoflonEOperation) cls.getEOperations().stream()
				.filter(x -> x.getName().equals("testPatternD")).findFirst().get();
		Assert.assertTrue("FailedAssert: 0", opB != null && opC != null && opD != null);
		StoryNode stnB = (StoryNode) opB.getActivity().getOwnedActivityNode().stream()
				.filter(x -> x instanceof StoryNode).collect(Collectors.toList()).get(0);
		StoryNode stnC = (StoryNode) opC.getActivity().getOwnedActivityNode().stream()
				.filter(x -> x instanceof StoryNode).collect(Collectors.toList()).get(0);
		StoryNode stnD = (StoryNode) opD.getActivity().getOwnedActivityNode().stream()
				.filter(x -> x instanceof StoryNode).collect(Collectors.toList()).get(0);
		...
		assertTrue(A.getGraphEdges().stream().filter(
				e -> e.getSource().getType().getName().equals("N") && e.getTarget().getType().getName().equals("Q"))
				.count() == 1);
		assertTrue(A.getGraphEdges().stream().filter(
				e -> e.getSource().getType().getName().equals("Q") && e.getTarget().getType().getName().equals("N"))
				.count() == 1);
