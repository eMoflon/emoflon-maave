...
public class JointlyEpiTest {
	...
	public void test1() {
		...
		MoflonEOperation opL1 = (MoflonEOperation) cls.getEOperations().stream()
				.filter(x -> x.getName().equals("ruleAddOne")).findFirst().get();
		MoflonEOperation opL2 = (MoflonEOperation) cls.getEOperations().stream()
				.filter(x -> x.getName().equals("ruleLarger4")).findFirst().get();
		Assert.assertTrue("FailedAssert: 0", opL1 != null && opL2 != null);
		StoryNode stnL1 = (StoryNode) opL1.getActivity().getOwnedActivityNode().stream()
				.filter(x -> x instanceof StoryNode).collect(Collectors.toList()).get(0);
		StoryNode stnL2 = (StoryNode) opL2.getActivity().getOwnedActivityNode().stream()
				.filter(x -> x instanceof StoryNode).collect(Collectors.toList()).get(0);
	...
	public void test2() {
		...
		System.out.println("number of jointlyEpiPairs1: " + jointliEpiPairSet.getMorphismPairs().size()
				+ ";;;; calculated in " + (time1end - time1start) + "ms");
		System.out.println("number of jointlyEpiPairs2: " + jointliEpiPairSet2.getMorphismPairs().size()
				+ ";;;; calculated in " + (time2end - time2start) + "ms");
		assertTrue(true);
