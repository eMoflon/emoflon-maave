...
public class DirectDerivationTest {
	...
	public void test1() {
		...
		MoflonEOperation opRule = (MoflonEOperation) cls.getEOperations().stream()
				.filter(x -> x.getName().equals("testRule1")).findFirst().get();
		...
		StoryNode stnRule = (StoryNode) opRule.getActivity().getOwnedActivityNode().stream()
		...
		DirectDerivationBuilder derBuilder = GraphtransformationFactory.eINSTANCE
				.createProjectiveDirectDerivationBuilder();
		...
		assertTrue(H.getGraphEdges().stream().anyMatch(
				x -> x.getSource().getType().getName().equals("N") && x.getTarget().getType().getName().equals("M")));
		assertTrue(H.getGraphEdges().stream().anyMatch(
				x -> x.getSource().getType().getName().equals("M") && x.getTarget().getType().getName().equals("N")));
		assertTrue(H.getGraphEdges().stream().anyMatch(
				x -> x.getSource().getType().getName().equals("M") && x.getTarget().getType().getName().equals("O")));

		assertTrue(H.getLabelNodes().size() == 4);

		assertTrue(H.getLabelEdges().size() == 3);
		assertTrue(H.getLabelEdges().stream().anyMatch(x -> x.getSource().getType().getName().equals("N")
				&& x.getTarget().getType().getName().equals("EInt")));
		assertTrue(H.getLabelEdges().stream().anyMatch(x -> x.getSource().getType().getName().equals("O")
				&& x.getTarget().getType().getName().equals("EInt")));
		assertTrue(H.getLabelEdges().stream().anyMatch(x -> x.getSource().getType().getName().equals("M")
				&& x.getTarget().getType().getName().equals("EInt")));
	...
	public void test2() {
		...
		MoflonEOperation opRule = (MoflonEOperation) cls.getEOperations().stream()
				.filter(x -> x.getName().equals("testRule1")).findFirst().get();
		MoflonEOperation opG = (MoflonEOperation) cls.getEOperations().stream()
				.filter(x -> x.getName().equals("testModel2")).findFirst().get();
		Assert.assertTrue("FailedAssert: 0", opRule != null && opG != null);
	...
	public void test3() {
		...
		MoflonEOperation opRule = (MoflonEOperation) cls.getEOperations().stream()
				.filter(x -> x.getName().equals("testRule1")).findFirst().get();
		MoflonEOperation opG = (MoflonEOperation) cls.getEOperations().stream()
				.filter(x -> x.getName().equals("testModel3")).findFirst().get();
		Assert.assertTrue("FailedAssert: 0", opRule != null && opG != null);
