...
public class GraphAndMorphismPrinter {
	...
	public static String print(SymbolicGraph graph) {
		...
		graph.getGraphNodes().forEach(x -> sb.append(print(x) + "\n"));
		sb.append("GraphEdges:\n");
		graph.getGraphEdges().forEach(x -> sb.append(print(x) + "\n"));
		sb.append("LabelNodes:\n");
		graph.getLabelNodes().forEach(x -> sb.append(print(x) + "\n"));
		sb.append("LabelEdges:\n");
		graph.getLabelEdges().forEach(x -> sb.append(print(x) + "\n"));
		sb.append("Formula:" + "\n");
	...
	public static String formatMapping(SymbolicGraph r1, String leftDelimiter, SymbolicGraph k1, String rightDelimiter,
			SymbolicGraph l12) {
		return formatMapping(r1, leftDelimiter, k1, rightDelimiter, l12, BASIC_OFFSET);
	}

	public static String formatMapping(GraphNode r1, String leftDelimiter, GraphNode k1, String rightDelimiter,
			GraphNode l12) {
		return formatMapping(r1, leftDelimiter, k1, rightDelimiter, l12, BASIC_OFFSET);
	}

	public static String formatMapping(LabelNode r1, String leftDelimiter, LabelNode k1, String rightDelimiter,
			LabelNode l12) {
		return formatMapping(r1, leftDelimiter, k1, rightDelimiter, l12, BASIC_OFFSET);
	}

	public static String formatMapping(GraphEdge r1, String leftDelimiter, GraphEdge k1, String rightDelimiter,
			GraphEdge l12) {
		return formatMapping(r1, leftDelimiter, k1, rightDelimiter, l12, BASIC_OFFSET);

	}

	public static String formatMapping(LabelEdge r1, String leftDelimiter, LabelEdge k1, String rightDelimiter,
			LabelEdge l12) {
		return formatMapping(r1, leftDelimiter, k1, rightDelimiter, l12, BASIC_OFFSET);
	}

	public static String formatMapping(SymbolicGraph r1, String leftDelimiter, SymbolicGraph k1, String rightDelimiter,
			SymbolicGraph l12, int offset) {
	...
	private static String formatMapping(String r1, String leftDelimiter, String k1, String rightDelimiter, String l12,
			int l) {

		return String.format("%-" + l + "s" + leftDelimiter + "%-" + l + "s" + rightDelimiter + "%-" + l + "s",
				StringUtils.leftPad(r1, l), StringUtils.center(k1, l), StringUtils.rightPad(l12, l));
	}
	...
	public static String internalPrintSpan(SymbolicGraphMorphism l, SymbolicGraphMorphism r) {
		...
		graphR.getGraphNodes().stream().filter(x -> !r.isInImage(x)).forEach(x -> {
			builder.append(formatMapping((GraphNode) null, LEFT_AR, (GraphNode) null, RIGHT_AR, x));
			builder.append("\n");
		});
		graphL.getGraphNodes().stream().filter(x -> !l.isInImage(x)).forEach(x -> {
		...
		graphR.getGraphEdges().stream().filter(x -> !r.isInImage(x)).forEach(x -> {
			builder.append(formatMapping((GraphEdge) null, LEFT_AR, (GraphEdge) null, RIGHT_AR, x));
			builder.append("\n");
		});
		graphL.getGraphEdges().stream().filter(x -> !l.isInImage(x)).forEach(x -> {
		...
		graphR.getLabelNodes().stream().filter(x -> !r.isInImage(x)).forEach(x -> {
			builder.append(formatMapping((LabelNode) null, LEFT_AR, (LabelNode) null, RIGHT_AR, x));
			builder.append("\n");
		});
		graphL.getLabelNodes().stream().filter(x -> !l.isInImage(x)).forEach(x -> {
		...
		graphR.getLabelEdges().stream().filter(x -> !r.isInImage(x)).forEach(x -> {
			builder.append(formatMapping((LabelEdge) null, LEFT_AR, (LabelEdge) null, RIGHT_AR, x));
			builder.append("\n");
		});
		graphL.getLabelEdges().stream().filter(x -> !l.isInImage(x)).forEach(x -> {
	...
	public static String internalPrintPair(SymbolicGraphMorphism first, SymbolicGraphMorphism second) {
		...
		builder.append("graphNodeMapping:" + "\n");
		graphK.getGraphNodes().stream().forEach(x -> {
			builder.append(formatMapping((GraphNode) getPreimage(x, first), RIGHT_AR, x, LEFT_AR,
					(GraphNode) getPreimage(x, second), offset));
			builder.append("\n");
		});
		builder.append("graphEdgesMapping:" + "\n");
		graphK.getGraphEdges().stream().forEach(x -> {
			builder.append(formatMapping((GraphEdge) getPreimage(x, first), RIGHT_AR, x, LEFT_AR,
					(GraphEdge) getPreimage(x, second), offset));
			builder.append("\n");
		});
		builder.append("labelNodeMapping:" + "\n");
		graphK.getLabelNodes().stream().forEach(x -> {
			builder.append(formatMapping((LabelNode) getPreimage(x, first), RIGHT_AR, x, LEFT_AR,
					(LabelNode) getPreimage(x, second), offset));
			builder.append("\n");
		});
		builder.append("labelEdgesMapping:" + "\n");
		graphK.getLabelEdges().stream().forEach(x -> {
			builder.append(formatMapping((LabelEdge) getPreimage(x, first), RIGHT_AR, x, LEFT_AR,
					(LabelEdge) getPreimage(x, second), offset));
			builder.append("\n");
		});
		return builder.toString();
	}
	...
	private static EGraphElement getPreimage(EGraphElement image, SymbolicGraphMorphism morphisms) {
		return (EGraphElement) morphisms.getDom().getAllElements().stream()
				.filter(elem -> morphisms.imageOf((EGraphElement) elem) == image).findAny().orElse(null);
	}
