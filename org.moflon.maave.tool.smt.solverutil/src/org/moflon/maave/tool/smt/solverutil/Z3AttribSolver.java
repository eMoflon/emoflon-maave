...
public class Z3AttribSolver implements IAttribSolver {
	...
	public boolean checkImplication(SymbolicGraphMorphism morphism) {
		...
		String smtStr = formulaTransformer.transformImplication(morphism.getCodom().getFormula(),
				morphism.getDom().getFormula(), ((SymbolicGraphMorphismImpl) morphism).labelNodeMap);
	...
	public boolean hasEquivalentFormulas(SymbolicGraphMorphism morphism) {
		...
		String smtStr = formulaTransformer.transformBiImplication(morphism.getCodom().getFormula(),
				morphism.getDom().getFormula(), ((SymbolicGraphMorphismImpl) morphism).labelNodeMap);
		Status result;
		try {
			result = check(smtStr);
