...
class JavaExample {
	...
	public void arrayExample3(Context ctx) throws Z3Exception, TestFailedException {
		...
		for (int n = 2; n <= 5; n++) {
			...
			if (n < 5) {
				for (int i = 0; i < n; i++) {
					System.out.println(a[i].toString() + " = " + model.evaluate(a[i], false));
				}
			}
		}
	}
	...
	public void quantifierExample3(Context ctx) throws Z3Exception, TestFailedException {
		...
		/*
		 * If quantified formulas are asserted in a logical context, then the model
		 * produced by Z3 should be viewed as a potential model.
		 */
	...
	public void pushPopExample1(Context ctx) throws Z3Exception, TestFailedException {
		...
		/*
		 * backtrack: the constraint x <= 3 will be removed, since it was asserted after
		 * the last ctx.Push.
		 */
