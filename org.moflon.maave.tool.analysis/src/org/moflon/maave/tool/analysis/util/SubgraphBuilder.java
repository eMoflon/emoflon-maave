				...
				for (int i = 0; i < graphEdges.size(); i++) {
					if (graphEdgesVec.testBit(graphEdges.size() - 1 - i)) {
						...
						if (graphNodesVec.testBit(graphNodes.size() - 1 - elemToIndexMap.get(imageEdge.getSource()))
								&& graphNodesVec
										.testBit(graphNodes.size() - 1 - elemToIndexMap.get(imageEdge.getTarget()))) {
							// remove wrong bidirectional EReffrences
							EReference opposite = imageEdge.getType().getEOpposite();
							if (opposite != null) {
								GraphEdge oppositeEdge = graphEdges.stream()
										.filter(e -> (e.getType() == opposite)
												&& (e.getSource() == imageEdge.getTarget())
												&& (e.getTarget() == imageEdge.getSource()))
										.findAny().get();
