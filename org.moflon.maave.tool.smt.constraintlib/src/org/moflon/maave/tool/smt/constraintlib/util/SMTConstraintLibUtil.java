...
public class SMTConstraintLibUtil {
	private static final URI BUILDIN_SMT_CONST_LIB_URI = URI
			.createPlatformPluginURI("/org.moflon.maave.tool.smt.constraintlib/lib/SMTConstraintLibrary.xmi", true);
	...
	private static Resource loadAttributeConstraintLibraryResource(URI uri, boolean createOnDemand) {
		...
		rset.getURIConverter().getURIMap().put(URI.createPlatformResourceURI("/", true),
				URI.createFileURI(System.getenv("CurrentWSLoc") + "\\"));
		rset.getURIConverter().getURIMap().put(
				URI.createPlatformPluginURI("/org.moflon.maave.tool.smt.constraintlib/", true),
				URI.createPlatformResourceURI("/org.moflon.maave.tool.smt.constraintlib/", true));
		Resource res = null;
